...
export default async function AdminPage() {
  ...
  const activeUsers = await sql`SELECT COUNT(*) as active FROM users WHERE is_active = true`
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{activeUsers[0].active}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {Number.parseInt(totalUsers[0].total) - Number.parseInt(activeUsers[0].active)}
            </div>
          </CardContent>
        </Card>
          ...
          <div className="space-y-4">
            {userStats.map((stat) => (
              <div key={stat.role} className="flex items-center justify-between p-4 border rounded-lg">
                <div>
                  <h3 className="font-semibold capitalize">{stat.role.replace("_", " ")}</h3>
